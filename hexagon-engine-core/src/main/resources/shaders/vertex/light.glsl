...
vec3 point_light_effect(vec3 surface_normal, vec3 world_position) {
	...
	float brightness = clamp(dot(unit_normal, to_light_vector), 0.2/*diffuse_light*/, 1.0);
	return brightness * light_color * 1.0/*light_intensity*/;
}
...
vec3 specular_light_effect(vec3 surface_normal, vec3 world_position, vec3 to_camera_vector) {
	...
	return damped_factor * reflectivity * light_color * 1.0/*light_intensity*/;
}